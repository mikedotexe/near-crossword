...
import { requestCampaignCancellation } from "../../../../../../src/server/v2/services";
...
export const POST = withErrors(async (
  ...
  await enforceRateLimit(`campaign-cancel:${clientAddress(request)}`, {
    limit: 10,
    windowMs: 60 * 60 * 1000,
  });
  const actor = await requireActor(request);
  const campaign = await requestCampaignCancellation(
    getRepository(),
    actor,
