...
| L09 | Base contract locally tested; integration pending | USDC on Base escrow and EIP-712 helper are implemented, not deployed. Crossword still validates only NEAR x402 networks and uses a NEAR browser payer. | Engineering integrates durable Base issuance/event ingestion and the EVM wallet/payment path in R3-R5; changing the facilitator URL alone cannot supply this. |
| L10 | Multi-recipient escrow locally tested; public workflow pending | Fixed-slot, recipient-bound claims, refunds, replay and stateful solvency tests pass. The live contract still has one winner. | Engineering connects private eligibility/account allocation, source review, sponsor/participant UX, and reconciliation before a reviewed small pilot. |
