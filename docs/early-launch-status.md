...
- 2026-09-05, session 13: Generated a separate local eligibility signer
  ...
  `.env.local` and macOS Keychain, and prepared the next exact approval preflight:
  native Base Sepolia USDC approval from deployer to escrow for `1000000` atomic
  units, nonce 1, suggested gas limit 77,488 and suggested max cost
  0.000000542416 test ETH. No approval, campaign funding, provider sponsorship
  request, fresh-wallet claim, production setting, mainnet transfer or credential
  print. See [campaign preflight](base-sepolia-campaign-preflight-2026-09-05.md).
