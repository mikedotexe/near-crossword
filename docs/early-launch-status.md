...
| L14 | Managed CDP path configured and implemented locally; funded acceptance pending | The CDP participant completed revision `2`, proved control of the counterfactual smart account and received campaign `3`'s signed 1-test-USDC authorization. Its custom-proxy retry is durably `UNKNOWN`; final checks found no deployment, nonce, balance or escrow-claim change. Session 25 saved Base Sepolia in the embedded-wallet Paymaster tab and implemented managed `useCdpPaymaster` with a one-shot database gas reservation, immutable provider reporting and finalized-event closure. | Use a fresh reviewed allocation for one managed payout and record provider cost, operation, transaction, finalized event, recipient delta and duplicate recovery. Do not retry campaign `3`. Keep production sponsorship disabled until that evidence passes. |
| L15 | Campaign 2 mismatch recovered; campaign 3 finalized, allocated and blocked from retry | Mike's standing approval is limited to small Base Sepolia test assets and incidental test gas. The application rejected campaign `2`'s noncanonical terms commitment before participant issuance; recovery transaction `0x865c8aaca90733d4b5ec1d390e66d3aa01287d1420559d4b37ce1cae7d704224` returned its full test USDC to the sponsor and finalized. Exact allowance `0x519ae06cd5269f368deab1bd7ab739c9ec8a20947c6c0207559307787cd3d319` and funding `0x4c9199cfaa7f8c138bc64da55a2ab7cddbe7a8eab77e9f7adfff99b13a1cbd99` created campaign `3` with canonical review hash `0x8ceb4b64f564424caf61e0957dc2bd090ce7cf315178f498468f1ed482d97ad8`. The participant allocation is preserved, but its gas outcome is unknown and cannot be retried. | Prepare a fresh exact-commitment Base Sepolia campaign/allocation for managed acceptance. Standing approval does not cover mainnet, production activation, materially larger spend, or submission. See the [campaign 3 record](base-sepolia-campaign-3-2026-09-07.md). |
| L16 | Public product surface live; Base Batches 004 application in preparation | The September 7 Render deployment `dep-dafgjup42hec73d9c3l0` serves the Base-first home, no-reward practice lesson, and non-persisting sponsor demo. Live checks returned 200 for those routes and `/legacy`; developer previews returned 404. The application closes September 9, 2026 and does not save drafts. Founder/team/contact/traction/financing facts are recorded offline. | Complete and publish Mike's video, reconcile every application claim to this register, and review before form submission. No application has been submitted. |
| L17 | Historical NEAR traction verified | A read-only FastNear audit of all 596 indexed function-call transactions for `crossword.puzzle.near` and `shitzu.crossword.puzzle.near` found 160 distinct successful solves and 157 exact successful payouts totaling 2,623 NEAR. All claim keys match solve keys and exact outbound receipt value. | Use only as historical legacy-product evidence. It does not establish unique humans, current Base users, sponsor revenue, or production readiness. See [chapter 12](../md-CLAUDE-chapters/12-near-mainnet-traction.md). |
...
- 2026-09-08, session 26: Added a repeatable read-only FastNear audit for both
  legacy mainnet contracts. Across 596 indexed function-call transactions it
  found 160 distinct successful puzzle solves and 157 distinct successful
  claims totaling exactly 2,623 NEAR. Every claim key matches a solve and every
  logged reward matches successful outbound receipt value, including nine
  account-creation deposits. Three solved puzzles were never successfully
  claimed; current final-state views show no unsolved puzzles. Updated the
  Batches materials with this historical evidence and explicit current-product
  qualifiers. No secret, transaction, deployment, or production setting changed.

- 2026-09-07, session 25: Saved a masked Base Sepolia Paymaster configuration in
  the CDP User Wallet project with blank context. Added migration 015 and a
  mutually exclusive managed mode using `useCdpPaymaster: true`. The server now
  reserves the bounded gas allowance before the SDK call, uses the reservation
  ID as CDP idempotency key, records one operation hash or durable uncertainty,
  and closes only from finalized reward evidence. Applied migration 015 and
  enabled managed mode only in ignored local acceptance configuration; disabled
  the local custom proxy. Render, production, mainnet and campaign funds were not
  changed. Campaign `3` remains blocked; a fresh allocation is required.
...
- 2026-09-07, session 22: The application correctly rejected campaign `2`'s
  ...
  recovery finalized with zero remaining liability. Replacement campaign `3`
  was prepared from approved review/layout commitments, funded with one test
  USDC, and reconciled at finalized block `46522071`. Exact local binding and
  publication plus an idempotent recheck passed. Production gates, mainnet funds,
  and the Batches form remain untouched; combined CDP participant acceptance is
  still open.

- 2026-09-07, session 21: Interpreted Mike's standing low-balance transfer
  approval conservatively as Base Sepolia test assets and incidental test gas.
  Added a fail-closed campaign harness and sent one transaction after simulation,
  exact-state checks and a `0.00001` test-ETH L2 execution-cost ceiling; the
  observed all-in fee including L1 data was `0.000001562510468571`. Transaction
  `0xb8c25e901974647e86c18ff98d227fed29da39c48c17862946191b9cf27ba803`
  created campaign `2` with one 1-test-USDC slot. Inclusion and finalized state
  reconciled at finalized block `46520808`; session 22 later recovered its full
  balance after the application binding check caught the terms mismatch. No
  duplicate transaction, production change, mainnet transfer or application
  submission occurred.
