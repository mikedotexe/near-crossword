...
| L09 | Base contract/accounting and real participant issuer locally tested; live acceptance pending | Canonical RPC/ledger guards now compose with persisted completion and EOA/deployed ERC-1271 verification. Local Anvil/Postgres proves two participant payouts and exact finalized receipt recovery. Nothing is deployed/configured; the browser x402 payer is still NEAR. | Engineering proves fresh Base Account/sponsored gas and EVM payment flows. Before activation, review deployment/RPC/code pins, finality freshness, supervised scanner cadence, scale and independent security evidence. |
| L10 | Review and participant APIs locally tested; public workflow pending | Approved funding is immutable; completion, expiring wallet challenges, durable claim issuance and private paid/recovery status pass local tests. `BASE_REVIEW_ENABLED`, `BASE_PARTICIPANT_ENABLED` and `BASE_CLAIM_ISSUANCE_ENABLED` default false. The live contract still has one winner. | Engineering builds safe publication/layout, owner funding-binding flow and sponsor/player UI, with full staged journey acceptance before a reviewed pilot. |
