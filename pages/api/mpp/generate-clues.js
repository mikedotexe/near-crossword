...
// Price per AI generation in USD
const AI_GENERATION_PRICE_USD = "0.10";
...
export default async function handler(req, res) {
  ...
  if (process.env.MPP_RECIPIENT) {
    ...
    const paymentHandler = mpp.charge({
      amount: AI_GENERATION_PRICE_USD,
      description: "AI crossword puzzle generation",
    });
  ...
  // Payment verified (or MPP not configured) — generate clues
  ...
  const objectiveLine = objective ? `Focus clues around: ${objective}` : "";
  ...
  if (hasPdf) {
    ...
    if (!videoId) {
      return res.status(400).json({ error: "Could not extract a video ID." });
    }
    ...
    if (!subtitles || subtitles.length === 0) {
      return res.status(400).json({ error: "No English transcript available." });
    }
  ...
  try {
    ...
    return res.status(200).json({ variations, paymentMethod: "tempo" });
  } catch (error) {
